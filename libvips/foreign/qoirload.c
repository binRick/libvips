...
#include "qoir/qoir.h"
#include "qoir/qoir/util/load_file.c"
#include "pforeign.h"
...
static gboolean
vips_foreign_load_qoir_is_a_source( VipsSource *source ){
...
static void
vips_foreign_load_qoir_dispose( GObject *gobject )
{
 ...
 */
static int
vips_foreign_load_qoir_parse_header( VipsForeignLoadQoir *qoir )
{
...
vips_foreign_load_qoir_get_flags( VipsForeignLoad *load )
{
...
static int
vips_foreign_load_qoir_header( VipsForeignLoad *load )
{
	VipsForeignLoadQoir *qoir = (VipsForeignLoadQoir *) load;
...
static void
vips_foreign_load_qoir_class_init( VipsForeignLoadQoirClass *class )
{
...
static void
vips_foreign_load_qoir_init( VipsForeignLoadQoir *qoir )
{
	qoir->scale = 1.0;
}
...
static gboolean
vips_foreign_load_qoir_file_is_a( const char *filename )
{
	VipsSource *source;
	gboolean result;
...
static int
vips_foreign_load_qoir_file_build( VipsObject *object )
{
...
static void
vips_foreign_load_qoir_file_class_init( VipsForeignLoadQoirClass *class )
{
	GObjectClass *gobject_class = G_OBJECT_CLASS( class );
	VipsObjectClass *object_class = (VipsObjectClass *) class;
	VipsForeignLoadClass *load_class = (VipsForeignLoadClass *) class;
...
static void
vips_foreign_load_qoir_file_init( VipsForeignLoadQoirFile *file )
{
}
...
static int
vips_foreign_load_qoir_source_build( VipsObject *object )
{
	VipsForeignLoadQoir *qoir = (VipsForeignLoadQoir *) object;
...
static void
vips_foreign_load_qoir_source_class_init( VipsForeignLoadQoirFileClass *class )
{
	GObjectClass *gobject_class = G_OBJECT_CLASS( class );
	VipsObjectClass *object_class = (VipsObjectClass *) class;
	VipsOperationClass *operation_class = VIPS_OPERATION_CLASS( class );
...
static void
vips_foreign_load_qoir_source_init( VipsForeignLoadQoirSource *source )
{
}
 ...
 */
int
vips_qoirload( const char *filename, VipsImage **out, ... )
{
	va_list ap;
	int result;
 ...
 */
int
vips_qoirload_source( VipsSource *source, VipsImage **out, ... )
{
	va_list ap;
	int result;
